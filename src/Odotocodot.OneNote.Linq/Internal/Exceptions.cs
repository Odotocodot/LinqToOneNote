...
namespace Odotocodot.OneNote.Linq.Internal
{
	...
	{
		internal static InvalidOperationException InvalidXmlElement(string elementName)
		{
			return new InvalidOperationException($"The XML element '{elementName}' is not valid in the current context.");
		}

		internal static InvalidOperationException InvalidXmlNodeType(XmlNodeType nodeType)
		{
		...
		{
			var description = GetErrorDescription((uint)ex.HResult);
			try
			{
				ref string message = ref GetExceptionMessageField(ex);
		...
		private static string GetErrorDescription(uint errorCode) => errorCode switch
		{
			...
			0x8004202F => "Returned by UpdatePageContent if IMConversationType page node property was to a value other than 0,1,2 or 3.",
			0x80042030 => "A modal dialog is blocking the app.",
			0x80042031 => "The publish format does not support sensitivity labeling.",
			_ => null,
		};
	}
}